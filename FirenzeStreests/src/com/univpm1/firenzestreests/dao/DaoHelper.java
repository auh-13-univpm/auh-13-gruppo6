...
public class DaoHelper extends SQLiteOpenHelper {
	...
	private static final String TABLE_INDIRIZZO = " CREATE TABLE  "
			...
			+ " nome varchar(60) not null ," + " latitudine varchar(15) ,"
			+ " longitudine varchar(15));";
	private static final String TABLE_SINISTRO = " CREATE TABLE sinistro(id_sinistro integer primary key autoincrement,"
			+ " anno integer , "
			+ " numero integer, "
			+ " id_via_Fk integer ,"
			+ " FOREIGN KEY(id_via_Fk) REFERENCES indirizzo(id_via));";
	private static final String TABLE_DANNO = " CREATE TABLE danno(id_danno integer primary key autoincrement ,"
			+ " lesioni integer ,"
			+ " contusi integer ,"
			+ " morti integer,"
			+ " id_via_Fk integer ,"
	...
	public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
		db.execSQL("DROP TABLE IF EXISTS sinistro");
		db.execSQL("DROP TABLE IF EXISTS danno");
